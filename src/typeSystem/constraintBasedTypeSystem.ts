import { CLOSURE_METHOD, CLOSURE_METHOD_NAME } from '../constants'
...
export const bindReceivedMessages = (tVar: TypeVariable): boolean => {
  ...
  for (const type of types) {
    for (const send of tVar.messages) {
      const message = send.message == CLOSURE_METHOD ? CLOSURE_METHOD_NAME : send.message // 'apply' is a special case for closures
